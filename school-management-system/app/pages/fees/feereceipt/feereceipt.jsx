...
const ReceiptGenerator = ({ receiptData, onClose, printDirect=false }) => {
  console.log("receiptData", receiptData);

  // Hard-coded school information
  const schoolInfo = {
    name: process.env.NEXT_PUBLIC_SCHOOL_NAME,
    phone: process.env.NEXT_PUBLIC_SCHOOL_PHONE,
    motto: process.env.NEXT_PUBLIC_SCHOOL_MOTTO,
...
if (printDirect) {
  ...
  const generateReceipt = () => {
    ...
    doc.setFontSize(18);
    doc.text(schoolInfo.name.toUpperCase(), pageWidth / 2, 12, {
      align: "center",
    });

    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.text(schoolInfo.phone, pageWidth / 2, 17, { align: "center" });

    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.text(schoolInfo.motto, pageWidth / 2, 24, { align: "center" });
